...
export function MySessionsClient({ sessions }: MySessionsClientProps) {
  ...
  const renderSessionCard = (session: Session, isUpcoming: boolean) => (
    <div 
      key={session.id} 
      className={`group relative bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] p-8 transition-all duration-500 hover:shadow-2xl ${isUpcoming ? 'hover:shadow-teal-500/10 hover:border-teal-500/30' : 'opacity-80 hover:opacity-100'}`}
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div className="flex items-center gap-5">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ${isUpcoming ? 'bg-teal-50 dark:bg-teal-900/20 text-teal-600' : 'bg-slate-50 dark:bg-slate-800 text-slate-500'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          {isUpcoming && (
            <span className="px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase bg-blue-500/10 text-blue-600 border border-blue-500/20 animate-pulse">
              Upcoming
            </span>
          )}
        </div>
    ...
    <div className="max-w-6xl mx-auto space-y-12">
      <section>
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Upcoming Confirmations</h2>
        </div>
