...
export default function GoogleLoginButton({ onBeforeClick, extraData, mode = 'login' }: GoogleLoginButtonProps) {
    ...
    try {
      ...
      if (data.success) {
        if (data.requiresRole) {
          ...
          // If we are on the profile page, just refresh instead of redirecting to dashboard
  ...
  }, [router, extraData, mode]);

  useEffect(() => {
    const initializeGoogleSignIn = () => {
      if (typeof window !== 'undefined' && window.google) {
        const clientInstance = window.google.accounts.oauth2.initCodeClient({
  ...
  const handleCustomClick = () => {
    ...
    if (client) {
      ...
      setError('Google client not initialized. Please refresh.');
    }
  };
